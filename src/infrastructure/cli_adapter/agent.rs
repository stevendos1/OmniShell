    ...
    pub fn new(config: CliAgentConfig) -> Result<Self> {
        if config.input_mode == InputMode::Arg && config.prompt_placeholder.is_none() {
            return Err(OrchestratorError::InvalidConfig(
                "input_mode 'arg' requires prompt_placeholder".into(),
            ));
        }
        let info = AgentInfo {
            id: config.id.clone(),
...
impl AiAgent for CliAgent {
    ...
    async fn execute(&self, request: AgentRequest) -> Result<AgentResponse> {
        ...
        let args = parse::build_args(
            ...
            &request.user_prompt,
            self.config.prompt_placeholder.as_deref(),
        );
        ...
        for env in &self.config.env_vars {
            let v = if env.value.starts_with('$') {
        ...
        let mut child = cmd.spawn().map_err(|e| OrchestratorError::AgentError {
            ...
            message: format!("spawn '{}': {e}", self.config.binary),
        ...
        if self.config.input_mode == InputMode::Stdin {
            if let Some(stdin) = child.stdin.as_mut() {
                ...
                stdin
                    ...
                    .await
                    .map_err(|e| OrchestratorError::agent(&self.info.id, format!("stdin: {e}")))?;
        ...
        let output = child
            .wait_with_output()
            .await
            .map_err(|e| OrchestratorError::agent(&self.info.id, format!("process: {e}")))?;
        ...
        let content = parse::parse_output(
            &stdout,
            self.config.output_format,
            &self.config.id,
            self.config.json_content_path.as_deref(),
        )?;
        Ok(AgentResponse {
            ...
            estimated_tokens: (stdout.len() as u64).div_ceil(4),
            duration,
            cache_hit: false,
            warnings: Vec::new(),
        })
    ...
    async fn health_check(&self) -> Result<()> {
        let r = tokio::process::Command::new("which")
        ...
        match r {
            ...
            Ok(_) => Err(OrchestratorError::agent(
                &self.info.id,
                format!("'{}' not in PATH", self.config.binary),
            )),
            Err(e) => Err(OrchestratorError::agent(
                &self.info.id,
                format!("health check: {e}"),
            )),
        }
    }
}
