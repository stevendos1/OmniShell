...
impl ToolExecutor for SecureToolExecutor {
    async fn execute(&self, request: ToolRequest) -> Result<ToolResponse> {
        if !self.config.enabled {
            return Err(OrchestratorError::ToolExecutionError("disabled".into()));
        }
        let bn = Self::basename(&request.command);
        if !self.is_allowed(bn) {
        ...
        let child = cmd.spawn().map_err(|e| {
            OrchestratorError::ToolExecutionError(format!("spawn '{}': {e}", request.command))
        })?;
        let output = timeout(ct, child.wait_with_output())
            .await
            .map_err(|_| OrchestratorError::Timeout {
                duration_ms: ct.as_millis() as u64,
        ...
        let mut t = false;
        let so = if rs.len() > self.config.max_stdout_bytes {
            t = true;
            warn!("stdout truncated");
            rs[..self.config.max_stdout_bytes].to_string()
        } else {
            rs.to_string()
        };
        let se = if re.len() > self.config.max_stderr_bytes {
            t = true;
            warn!("stderr truncated");
            re[..self.config.max_stderr_bytes].to_string()
        } else {
            re.to_string()
        };
        Ok(ToolResponse {
    ...
    fn is_allowed(&self, command: &str) -> bool {
        ...
        if self.config.denied_commands.contains(bn) {
            return false;
        }
