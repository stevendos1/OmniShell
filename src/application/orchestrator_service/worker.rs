...
pub(crate) async fn execute_spawned(
    ...
    cb: CircuitBreaker,
    cache: Arc<dyn Cache>,
    ctx_mgr: Arc<dyn ContextManager>,
    rp: RetryPolicy,
    tp: TimeoutPolicy,
    cfg_ver: String,
    ...
    if let Ok(Some(cached)) = cache.get(&ck).await {
        if let Ok(r) = serde_json::from_str::<AgentResponse>(&cached.value) {
            return Ok(AgentResponse { cache_hit: true, ..r });
        }
    }
    let sp = ctx_mgr.build_system_prompt(&session_id).await.unwrap_or_default();
    let ar = AgentRequest {
    ...
    for attempt in 0..=rp.max_retries {
        if attempt > 0 {
            let d = Duration::from_millis((rp.base_delay.as_millis() as f64 * rp.backoff_multiplier.powi(attempt as i32 - 1)) as u64).min(rp.max_delay);
        ...
        match timeout(t, agent.execute(ar.clone())).await {
            Ok(Ok(resp)) => {
                ...
                if let Ok(s) = serde_json::to_string(&resp) {
                    let entry = CacheEntry { value: s, created_at: chrono::Utc::now().timestamp(), hit_count: 0, byte_size: resp.content.len() };
            ...
            Err(_) => {
                cb.record_failure(&aid).await;
                last_err = Some(OrchestratorError::Timeout { duration_ms: t.as_millis() as u64, context: format!("agent {aid}") });
            }
        }
    }
