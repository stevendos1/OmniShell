...
fn make_manager() -> InMemoryContextManager {
    InMemoryContextManager::new(ContextConfig { max_messages: 3, max_bytes: 10_000, max_tokens: 500, enable_summarization: false }, Arc::new(SimpleTokenCounter))
}

fn make_msg(role: MessageRole, content: &str) -> Message {
    Message { role, content: content.into(), token_estimate: (content.len() as u64 + 3) / 4, timestamp: 0 }
}

#[tokio::test]
async fn test_add_and_retrieve() {
    let mgr = make_manager();
    mgr.add_message("s1", make_msg(MessageRole::User, "Hello")).await.unwrap();
...
async fn test_trim_by_count() {
    let mgr = make_manager();
    for i in 0..5 {
        mgr.add_message("s1", make_msg(MessageRole::User, &format!("msg-{i}"))).await.unwrap();
    }
...
async fn test_ledger() {
    let mgr = make_manager();
    mgr.add_ledger_entry("s1", LedgerEntry { kind: LedgerEntryKind::Fact, key: "lang".into(), value: "Rust".into(), timestamp: 0, source_agent: None }).await.unwrap();
